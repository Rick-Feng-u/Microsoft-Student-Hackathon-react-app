...
describe('Style flatten and merge tests', function () {
    test('flatten recursive arrays', function () {
        var flattened = mergeStyles_1.flattenStyle(s1);
        expect(flattened).toEqual(s1flatten);
        expect(flattened).not.toBe(s1);
    });
    test('flatten flat style returns style', function () {
        var flattened = mergeStyles_1.flattenStyle(s2);
        expect(flattened).toBe(s2);
    });
    test('merge also flattens', function () {
        var merged = mergeStyles_1.mergeAndFlattenStyles(undefined, undefined, s1, s2);
        expect(merged).toEqual(sMerged);
    });
    test('merge with sub objects', function () {
        var merged = mergeStyles_1.mergeAndFlattenStyles(undefined, undefined, sSelector, sSelector2);
        expect(merged).toEqual(sMergedSelectors);
    });
    test('merge sub objects in arrays', function () {
        var merged = mergeStyles_1.mergeAndFlattenStyles(undefined, undefined, sArraySelector, sArraySelector2);
        expect(merged).toEqual(sMergedSelectors);
    });
    test('memo recursive arrays', function () {
        var flattened = mergeStyles_1.mergeStyles(s1);
        var flattened2 = mergeStyles_1.mergeStyles(s1);
        expect(flattened).toEqual(s1flatten);
        expect(flattened2).toBe(flattened);
    });
    test('memo flat style', function () {
        var flattened = mergeStyles_1.mergeStyles(s2);
        var flattened2 = mergeStyles_1.mergeStyles(s2);
        expect(flattened).toBe(s2);
        expect(flattened2).toBe(flattened);
    });
    test('memo and flatten multiple', function () {
        var flattened = mergeStyles_1.mergeStyles(s1, s2);
        var flattened2 = mergeStyles_1.mergeStyles(s1, s2);
        expect(flattened).toEqual(sMerged);
        expect(flattened2).toBe(flattened);
    });
    test('memo styles ignores undefined values', function () {
        var result1 = mergeStyles_1.mergeStyles(s1, s2, undefined, s1flatten);
        var result2 = mergeStyles_1.mergeStyles(s1, undefined, s2, s1flatten);
